...
void PawnSimApi::toggleTrace()
{
    ...
    if (!state_.tracing_enabled)
        ...
        // For physics-driven pawns (e.g. Car), last_position is never updated via setPose.
        // Initialize it so trace starts from current position when toggled on.
        state_.last_position = getUUPosition();
    }
}
...
{
    unused(dt);
    // Draw trace for physics-driven pawns (e.g. Car) that move via UE physics rather than setPose.
    // Multirotor overrides updateRendering and draws trace via setPoseInternal instead.
    FVector position = getUUPosition();
    float height_offset = getTraceHeightOffset();
    FVector trace_start = state_.last_position + FVector(0, 0, height_offset);
    FVector trace_end = position + FVector(0, 0, height_offset);
    if (state_.tracing_enabled && (state_.last_position - position).SizeSquared() > 0.25) {
        DrawDebugLine(params_.pawn->GetWorld(), trace_start, trace_end, trace_color_, true, -1.0F, 0, trace_thickness_);
        state_.last_position = position;
    }
    else if (!state_.tracing_enabled) {
        state_.last_position = position;
