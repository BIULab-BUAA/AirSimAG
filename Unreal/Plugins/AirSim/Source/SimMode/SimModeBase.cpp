...
void ASimModeBase::BeginPlay()
{
    ...
    // Create WorldSimApi first (with nullptr for api_provider temporarily)
    world_sim_api_.reset(new WorldSimApi(nullptr, this));
    // Create ApiProvider with WorldSimApi
    api_provider_.reset(new msr::airlib::ApiProvider(world_sim_api_.get()));
    // Now update WorldSimApi with ApiProvider reference
    // Cast to WorldSimApi* to access setApiProvider method
    WorldSimApi* world_sim_api_ptr = static_cast<WorldSimApi*>(world_sim_api_.get());
    if (world_sim_api_ptr) {
        world_sim_api_ptr->setApiProvider(api_provider_.get());
    }
