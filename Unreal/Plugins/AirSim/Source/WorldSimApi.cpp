...
#include <ctime>
#include <algorithm>
WorldSimApi::WorldSimApi(msr::airlib::ApiProvider* api_provider, ASimModeBase* simmode)
    : api_provider_(api_provider), simmode_(simmode) {}
bool WorldSimApi::loadLevel(const std::string& level_name)
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, level_name, world]() {
        this->current_level_ = UAirBlueprintLib::loadLevel(world, FString(level_name.c_str()));
    },
                                             true);
...
void WorldSimApi::spawnPlayer()
{
    ...
    UWorld* world = getWorld();
    if (!world) {
        UE_LOG(LogTemp, Error, TEXT("Could not get World for spawning player"));
        return;
    }
    UAirBlueprintLib::RunCommandOnGameThread([&success, world]() {
        success = UAirBlueprintLib::spawnPlayer(world);
    },
                                             true);
    if (!success) {
...
std::string WorldSimApi::spawnObject(const std::string& object_name, const std::string& load_object, const WorldSimApi::Pose& pose, const WorldSimApi::Vector3r& scale, bool physics_enabled, bool is_blueprint)
{
    ...
    FTransform actor_transform = getGlobalNedTransform().fromGlobalNed(pose);
...
AActor* WorldSimApi::createNewStaticMeshActor(const FActorSpawnParameters& spawn_params, const FTransform& actor_transform, const Vector3r& scale, UStaticMesh* static_mesh)
{
    ...
    AActor* NewActor = world->SpawnActor<AActor>(AActor::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, spawn_params);
    if (IsValid(NewActor)) {
...
AActor* WorldSimApi::createNewBPActor(const FActorSpawnParameters& spawn_params, const FTransform& actor_transform, const Vector3r& scale, UBlueprint* blueprint)
{
    ...
    UWorld* world = getWorld();
    if (!world) return nullptr;
    AActor* new_actor = world->SpawnActor<AActor>(new_bp, FVector::ZeroVector, FRotator::ZeroRotator, spawn_params);
    if (new_actor) {
...
bool WorldSimApi::createVoxelGrid(const Vector3r& position, const int& x_size, const int& y_size, const int& z_size, const float& res, const std::string& output_file)
{
    ...
    params.TraceTag = "";
    auto position_in_UE_frame = getGlobalNedTransform().fromGlobalNed(position);
    for (float i = 0; i < ncells_x; i++) {
        for (float k = 0; k < ncells_z; k++) {
            for (float j = 0; j < ncells_y; j++) {
                ...
                UWorld* world = getWorld();
                if (world) {
                    voxel_grid_[idx] = world->OverlapBlockingTestByChannel(vposition, FQuat::Identity, ECollisionChannel::ECC_Pawn, FCollisionShape::MakeBox(FVector(scale_cm / 2)), params);
                }
            }
        }
    }
...
WorldSimApi::Pose WorldSimApi::getObjectPose(const std::string& object_name) const
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &object_name, &result]() {
        ...
        result = actor ? getGlobalNedTransform().toGlobalNed(FTransform(actor->GetActorRotation(), actor->GetActorLocation()))
                       : Pose::nanPose();
    },
                                             true);
    return result;
...
void WorldSimApi::enableWeather(bool enable)
{
    UAirBlueprintLib::RunCommandOnGameThread([this, enable]() {
        UWorld* world = getWorld();
        if (world) {
            UWeatherLib::setWeatherEnabled(world, enable);
        }
    },
                                             true);
}
void WorldSimApi::setWeatherParameter(WeatherParameter param, float val)
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, param_e, val]() {
        UWorld* world = getWorld();
        if (world) {
            UWeatherLib::setWeatherParamScalar(world, param_e, val);
        }
    },
                                             true);
}
...
void WorldSimApi::simFlushPersistentMarkers()
{
    UWorld* world = getWorld();
    if (world) {
        UAirBlueprintLib::RunCommandOnGameThread([world]() {
...
void WorldSimApi::simPlotPoints(const std::vector<Vector3r>& points, const std::vector<float>& color_rgba, float size, float duration, bool is_persistent)
{
    ...
    UWorld* world = getWorld();
    if (!world) return;
    UAirBlueprintLib::RunCommandOnGameThread([this, &points, &color, size, duration, is_persistent, world]() {
        for (const auto& point : points) {
            DrawDebugPoint(world,
                           getGlobalNedTransform().fromGlobalNed(point),
                           size,
                           color,
                           is_persistent,
                           duration);
...
void WorldSimApi::simPlotLineStrip(const std::vector<Vector3r>& points, const std::vector<float>& color_rgba, float thickness, float duration, bool is_persistent)
{
    ...
    UWorld* world = getWorld();
    if (!world) return;
    UAirBlueprintLib::RunCommandOnGameThread([this, &points, &color, thickness, duration, is_persistent, world]() {
        for (size_t idx = 0; idx != points.size() - 1; ++idx) {
            DrawDebugLine(world,
                          getGlobalNedTransform().fromGlobalNed(points[idx]),
                          getGlobalNedTransform().fromGlobalNed(points[idx + 1]),
                          color,
                          is_persistent,
                          duration,
                          0,
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &points_start, &points_end, &color, thickness, arrow_size, duration, is_persistent]() {
        for (int idx = 0; idx < points_start.size(); ++idx) {
            DrawDebugDirectionalArrow(getWorld(),
                                      getGlobalNedTransform().fromGlobalNed(points_start[idx]),
                                      getGlobalNedTransform().fromGlobalNed(points_end[idx]),
                                      arrow_size,
                                      color,
                                      is_persistent,
                                      duration,
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &strings, &positions, &color, scale, duration]() {
        for (int idx = 0; idx < positions.size(); ++idx) {
            DrawDebugString(getWorld(),
                            getGlobalNedTransform().fromGlobalNed(positions[idx]),
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &poses, scale, thickness, duration, is_persistent]() {
        for (const auto& pose : poses) {
            DrawDebugCoordinateSystem(getWorld(),
                                      getGlobalNedTransform().fromGlobalNed(pose.position),
                                      getGlobalNedTransform().fromNed(pose.orientation).Rotator(),
                                      scale,
                                      is_persistent,
                                      duration,
                                      0,
...
void WorldSimApi::simPlotTransformsWithNames(const std::vector<Pose>& poses, const std::vector<std::string>& names, float tf_scale, float tf_thickness, float text_scale, const std::vector<float>& text_color_rgba, float duration)
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &poses, &names, &color, tf_scale, tf_thickness, text_scale, duration]() {
        for (int idx = 0; idx < poses.size(); ++idx) {
            DrawDebugCoordinateSystem(getWorld(),
                                      getGlobalNedTransform().fromGlobalNed(poses[idx].position),
                                      getGlobalNedTransform().fromNed(poses[idx].orientation).Rotator(),
                                      tf_scale,
                                      false,
                                      duration,
                                      0,
                                      tf_thickness);
            DrawDebugString(getWorld(),
                            getGlobalNedTransform().fromGlobalNed(poses[idx]).GetLocation(),
...
bool WorldSimApi::testLineOfSightBetweenPoints(const msr::airlib::GeoPoint& lla1, const msr::airlib::GeoPoint& lla2) const
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &lla1, &lla2, &hit]() {
        ...
        hit = getWorld()->LineTraceTestByChannel(point1, point2, ECC_Visibility, collision_params);
        if (settings.show_los_debug_lines_) {
            ...
            getWorld()->PersistentLineBatcher->DrawLine(point1, point2, color, SDPG_World, 4, 999999);
        }
    },
                                             true);
    return !hit;
...
msr::airlib::CameraInfo WorldSimApi::getCameraInfo(const CameraDetails& camera_details) const
{
    ...
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
...
void WorldSimApi::setCameraPose(const msr::airlib::Pose& pose, const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, &pose]() {
            mutable_camera->setCameraPose(pose);
        },
                                                 true);
    }
}
void WorldSimApi::setCameraFoV(float fov_degrees, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, &fov_degrees]() {
            mutable_camera->setCameraFoV(fov_degrees);
        },
                                                 true);
    }
}
void WorldSimApi::setDistortionParam(const std::string& param_name, float value, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, &param_name, &value]() {
            mutable_camera->setDistortionParam(param_name, value);
        },
                                                 true);
    }
}
    ...
    const std::vector<ImageCaptureBase::ImageRequest>& requests, const std::string& vehicle_name, bool external) const
{
    ...
    const UnrealImageCapture* camera = getImageCaptureInternal(vehicle_name, external);
    if (camera) {
        camera->getImages(requests, responses);
    } else {
        // Return empty responses if camera is not available
        for (unsigned int i = 0; i < requests.size(); ++i) {
            responses.push_back(ImageCaptureBase::ImageResponse());
            responses[responses.size() - 1].message = "camera is not available";
        }
    }
    return responses;
}
...
std::vector<std::string> WorldSimApi::getPresetLensSettings(const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        return camera->getPresetLensSettings();
    }
...
void WorldSimApi::setPresetLensSettings(std::string preset, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->setPresetLensSettings(preset);
    }
}
std::vector<std::string> WorldSimApi::getPresetFilmbackSettings(const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        return camera->getPresetFilmbackSettings();
    }
    return std::vector<std::string>();
}
void WorldSimApi::setPresetFilmbackSettings(std::string preset, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->setPresetFilmbackSettings(preset);
    }
}
std::string WorldSimApi::getFilmbackSettings(const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        return camera->getFilmbackSettings();
    }
    return std::string();
}
float WorldSimApi::setFilmbackSettings(float width, float height, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        return mutable_camera->setFilmbackSettings(width, height);
    }
    return 0.0f;
}
...
void WorldSimApi::setFocalLength(float focal_length, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->setFocalLength(focal_length);
    }
}
void WorldSimApi::enableManualFocus(bool enable, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->enableManualFocus(enable);
    }
}
...
void WorldSimApi::setFocusDistance(float focus_distance, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->setFocusDistance(focus_distance);
    }
}
float WorldSimApi::getFocusAperture(const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        return camera->getFocusAperture();
    }
    return 0.0f;
}
void WorldSimApi::setFocusAperture(float focus_aperture, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->setFocusAperture(focus_aperture);
    }
}
void WorldSimApi::enableFocusPlane(bool enable, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        mutable_camera->enableFocusPlane(enable);
    }
}
std::string WorldSimApi::getCurrentFieldOfView(const CameraDetails& camera_details)
{
    const APIPCamera* camera = getCameraInternal(camera_details);
    if (camera) {
        return camera->getCurrentFieldOfView();
    }
    return std::string();
}
//End CinemAirSim
void WorldSimApi::addDetectionFilterMeshName(ImageCaptureBase::ImageType image_type, const std::string& mesh_name, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, image_type, &mesh_name]() {
            mutable_camera->getDetectionComponent(image_type, false)->addMeshName(mesh_name);
        },
                                                 true);
    }
}
void WorldSimApi::setDetectionFilterRadius(ImageCaptureBase::ImageType image_type, float radius_cm, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, image_type, radius_cm]() {
            mutable_camera->getDetectionComponent(image_type, false)->setFilterRadius(radius_cm);
        },
                                                 true);
    }
}
void WorldSimApi::clearDetectionMeshNames(ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details)
{
    ...
    if (camera) {
        APIPCamera* mutable_camera = const_cast<APIPCamera*>(camera);
        UAirBlueprintLib::RunCommandOnGameThread([mutable_camera, image_type]() {
            mutable_camera->getDetectionComponent(image_type, false)->clearMeshNames();
        },
                                                 true);
    }
}
std::vector<msr::airlib::DetectionInfo> WorldSimApi::getDetections(ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details)
{
    ...
    // Get NedTransform
    const NedTransform* ned_transform_ptr = nullptr;
    if (camera_details.external) {
        if (simmode_) {
            ned_transform_ptr = &getGlobalNedTransform();
        }
    } else {
        msr::airlib::ApiProvider* provider = api_provider_;
        if (!provider && simmode_) {
            provider = simmode_->getApiProvider();
        }
        if (provider) {
            auto vehicle_sim_api = provider->getVehicleSimApi(camera_details.vehicle_name);
            if (!vehicle_sim_api) {
                vehicle_sim_api = provider->getVehicleSimApi("");
            }
            if (vehicle_sim_api) {
                auto* pawn_sim_api = static_cast<PawnSimApi*>(vehicle_sim_api);
                if (pawn_sim_api) {
                    ned_transform_ptr = &pawn_sim_api->getNedTransform();
                }
            }
        }
    }
    if (!ned_transform_ptr) {
        return result;
    }
    const NedTransform& ned_transform = *ned_transform_ptr;
...
// Helper methods
UWorld* WorldSimApi::getWorld() const
{
    if (simmode_) {
        return simmode_->GetWorld();
    }
    // Try to get world from first vehicle
    if (api_provider_ && api_provider_->getVehicleCount() > 0) {
        auto& vehicle_sim_apis = api_provider_->getVehicleSimApis();
        if (vehicle_sim_apis.valsSize() > 0) {
            auto it = vehicle_sim_apis.begin();
            if (it != vehicle_sim_apis.end()) {
                auto* vehicle_sim_api = *it;
                auto* pawn_sim_api = static_cast<PawnSimApi*>(vehicle_sim_api);
                if (pawn_sim_api && pawn_sim_api->getPawn()) {
                    return pawn_sim_api->getPawn()->GetWorld();
                }
            }
        }
    }
...
const NedTransform& WorldSimApi::getGlobalNedTransform() const
{
    if (simmode_) {
        return simmode_->getGlobalNedTransform();
    }
    // Try to get from first vehicle
    msr::airlib::ApiProvider* provider = api_provider_;
    if (!provider && simmode_) {
        provider = simmode_->getApiProvider();
    }
    if (provider && provider->getVehicleCount() > 0) {
        auto& vehicle_sim_apis = provider->getVehicleSimApis();
        if (vehicle_sim_apis.valsSize() > 0) {
            auto it = vehicle_sim_apis.begin();
            if (it != vehicle_sim_apis.end()) {
                auto* vehicle_sim_api = *it;
                auto* pawn_sim_api = static_cast<PawnSimApi*>(vehicle_sim_api);
                if (pawn_sim_api) {
                    return pawn_sim_api->getNedTransform();
                }
            }
        }
    }
    // Fallback: return a default transform (this should not happen in normal operation)
    static NedTransform default_transform(FTransform::Identity, 100.0f);
    return default_transform;
}
const UnrealImageCapture* WorldSimApi::getImageCaptureInternal(const std::string& vehicle_name, bool external) const
{
    if (external) {
        // For external cameras, try to get from simmode if available
        if (simmode_) {
            return simmode_->getExternalImageCapture();
        }
        // If no simmode, external cameras may not be available
        return nullptr;
    }
    // For vehicle cameras, get from VehicleSimApi
    // Try to get api_provider from simmode if api_provider_ is not set
    msr::airlib::ApiProvider* provider = api_provider_;
    if (!provider && simmode_) {
        provider = simmode_->getApiProvider();
    }
    ...
    auto vehicle_sim_api = provider->getVehicleSimApi(vehicle_name);
    if (!vehicle_sim_api) {
        // Try default vehicle if vehicle_name is empty
        vehicle_sim_api = provider->getVehicleSimApi("");
    }
    if (vehicle_sim_api) {
        return static_cast<const UnrealImageCapture*>(vehicle_sim_api->getImageCapture());
    }
    return nullptr;
}
const APIPCamera* WorldSimApi::getCameraInternal(const CameraDetails& camera_details) const
{
    if (camera_details.external) {
        // For external cameras, try to get from simmode if available
        if (simmode_) {
            return simmode_->getExternalCamera(camera_details.camera_name);
        }
        return nullptr;
    }
    // For vehicle cameras, get from VehicleSimApi
    ...
    if (vehicle_sim_api) {
        auto* pawn_sim_api = static_cast<PawnSimApi*>(vehicle_sim_api);
        if (pawn_sim_api) {
            return pawn_sim_api->getCamera(camera_details.camera_name);
        }
    }
    return nullptr;
}
