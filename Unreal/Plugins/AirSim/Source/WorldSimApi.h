...
public:
    ...
    WorldSimApi(msr::airlib::ApiProvider* api_provider, ASimModeBase* simmode = nullptr);
    virtual ~WorldSimApi() = default;
    
    void setApiProvider(msr::airlib::ApiProvider* api_provider) { api_provider_ = api_provider; }
...
private:
    ...
    // Helper methods to get ImageCapture and Camera
    const UnrealImageCapture* getImageCaptureInternal(const std::string& vehicle_name, bool external) const;
    const APIPCamera* getCameraInternal(const CameraDetails& camera_details) const;
    UWorld* getWorld() const;
    const NedTransform& getGlobalNedTransform() const;

private:
    msr::airlib::ApiProvider* api_provider_;
    ASimModeBase* simmode_;  // Optional, for backward compatibility and World access
