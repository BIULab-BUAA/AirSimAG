    ...
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if (isENU_) {
            try {
                ...
                auto transformStampedENU = tf_buffer_.lookupTransform(AIRSIM_FRAME_ID, "body", ros::Time(0), ros::Duration(1));
...
void AirsimROSWrapper::append_static_vehicle_tf(VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting)
{
    ...
    if (!body_frame_added) {
        ...
        geometry_msgs::TransformStamped body_tf_msg;
        body_tf_msg.header.frame_id = vehicle_ros->odom_frame_id;
        ...
        vehicle_ros->static_tf_msg_vec.emplace_back(body_tf_msg);
    }
}
