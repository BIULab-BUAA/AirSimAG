...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& [vehicle_name, vehicle_setting] : get_settings().vehicles) {
        ...
        if (msr::airlib::AirSimSettings::isMultirotor(vehicle_setting->vehicle_type)) {
            ...
            drone->vel_cmd_body_frame_sub_ = nh_->create_subscription<airsim_interfaces::msg::VelCmd>(topic_prefix + "/vel_cmd_body_frame", 1, fcn_vel_cmd_body_frame_sub);
...
bool AirsimROSWrapper::takeoff_srv_cb(std::shared_ptr<airsim_interfaces::srv::Takeoff::Request> request, std::shared_ptr<airsim_interfaces::srv::Takeoff::Response> response, const std::string& vehicle_name)
{
    ...
    if (request->wait_on_last_task)
        get_multirotor_client()->takeoffAsync(20, vehicle_name)->waitOnLastTask();
    // response->success =
    else
...
bool AirsimROSWrapper::takeoff_all_srv_cb(std::shared_ptr<airsim_interfaces::srv::Takeoff::Request> request, std::shared_ptr<airsim_interfaces::srv::Takeoff::Response> response)
{
    ...
    if (request->wait_on_last_task)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            get_multirotor_client()->takeoffAsync(20, vehicle_name_ptr_pair.first)->waitOnLastTask();
    // response->success =
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
...
// 3. call airsim client's setCameraPose which sets camera pose wrt world (or takeoff?) ned frame.
...
sensor_msgs::msg::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name) const
{
    ...
    lidar_msg.header.frame_id = "body";
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if (isENU_) {
            try {
                ...
                auto transformStampedENU = tf_buffer_->lookupTransform(AIRSIM_FRAME_ID, "body", rclcpp::Time(0), rclcpp::Duration(1));
...
sensor_msgs::msg::Imu AirsimROSWrapper::get_imu_msg_from_airsim(const msr::airlib::ImuBase::Output& imu_data) const
{
    ...
    // imu_msg.header.frame_id = "/airsim/odom_local_ned";
...
void AirsimROSWrapper::append_static_vehicle_tf(VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting)
{
    ...
    // body frame: identity with odom body frame (only first vehicle to avoid multiple parents)
    static bool body_frame_added = false;
    if (!body_frame_added) {
        body_frame_added = true;
        geometry_msgs::msg::TransformStamped body_tf_msg;
        body_tf_msg.header.frame_id = vehicle_ros->odom_frame_id_;
        body_tf_msg.child_frame_id = "body";
        body_tf_msg.transform.translation.x = 0;
        body_tf_msg.transform.translation.y = 0;
        body_tf_msg.transform.translation.z = 0;
        body_tf_msg.transform.rotation.x = 0;
        body_tf_msg.transform.rotation.y = 0;
        body_tf_msg.transform.rotation.z = 0;
        body_tf_msg.transform.rotation.w = 1;
        vehicle_ros->static_tf_msg_vec_.emplace_back(body_tf_msg);
    }
}
...
void AirsimROSWrapper::img_response_timer_cb()
{
    try {
        ...
        for (const auto& airsim_img_request_vehicle_name_pair : airsim_img_request_vehicle_name_pair_vec_) {
            const std::string& vehicle_name = airsim_img_request_vehicle_name_pair.second;
            auto it = vehicle_name_ptr_map_.find(vehicle_name);
            if (it == vehicle_name_ptr_map_.end())
                continue;
            const std::vector<ImageResponse>& img_response = get_client(it->second->vehicle_type_)->simGetImages(airsim_img_request_vehicle_name_pair.first, vehicle_name);
    ...
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs_.empty()) {
                auto* client = get_client(vehicle_name_ptr_pair.second->vehicle_type_);
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs_) {
                    auto lidar_data = client->getLidarData(lidar_publisher.sensor_name, vehicle_name_ptr_pair.first);
